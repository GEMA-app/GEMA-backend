...
from app.application.dtos.inventory_part_dtos import UpdateInventoryPartRequest, InventoryPartResponse
from app.application.ports.unit_of_work import UnitOfWorkPort
from app.domain.exceptions import InventoryPartNotFoundError, StaleDataError  # Asumiendo que existen en tus exceptions
from app.domain.value_objects import CompanyId, InventoryPartId, ProviderId
...
class UpdateInventoryPartUseCase:
    ...
    async def execute(
        ...
        part_id = InventoryPartId.from_string(part_id_str)

        async with self.uow:
            # 1. Obtener la entidad actual
            inventory_part = await self.uow.inventory_parts.get_by_id(company_id, part_id)
