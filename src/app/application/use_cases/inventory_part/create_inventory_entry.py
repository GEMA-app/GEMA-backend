...
import uuid
from app.application.dtos.inventory_part_dtos import CreateInventoryEntryRequest, InventoryEntryResponse
...
class CreateInventoryEntryUseCase:
    ...
    async def execute(
        ...
        repuesto_uuid = uuid.UUID(request.repuesto_id)

        async with self.uow:
            # 1. Buscar el repuesto en el repositorio (entidad de dominio)
            inventory_part = await self.uow.inventory_parts.get_by_id(repuesto_uuid)
            if not inventory_part:
                raise ValueError(f"No se encontró el repuesto con ID: {request.repuesto_id}")
