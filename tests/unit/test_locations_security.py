...
def test_get_location_tree_uses_require_tenant_read() -> None:
    dep = _get_dependency(get_location_tree, "current_user")
    assert dep is require_tenant_read


def test_get_location_uses_require_tenant_read() -> None:
    dep = _get_dependency(get_location, "current_user")
    assert dep is require_tenant_read


def test_get_location_children_uses_require_tenant_read() -> None:
    dep = _get_dependency(get_location_children, "current_user")
    assert dep is require_tenant_read


def test_get_location_uses_location_id_param() -> None:
    sig = inspect.signature(get_location)
    assert "ubicacion_id" in sig.parameters


def test_update_location_uses_location_id_param() -> None:
    sig = inspect.signature(update_location)
    assert "ubicacion_id" in sig.parameters


def test_delete_location_uses_location_id_param() -> None:
    sig = inspect.signature(delete_location)
    assert "ubicacion_id" in sig.parameters


def test_get_location_children_uses_location_id_param() -> None:
    sig = inspect.signature(get_location_children)
    assert "ubicacion_id" in sig.parameters


def test_create_location_keeps_require_permission() -> None:
    dep = _get_dependency(create_location, "current_user")
    assert dep is not None
    assert callable(dep)


def test_update_location_keeps_require_permission() -> None:
